...
//------------------------Dean utilities ---------------------------------
 ...
 int main()
   {
      string imagefilename = "noisy.gif";
		 ...
		 { 
			 for (int y=1;y<img.height()-1;y++)
			 {
				 for (int i=0;i<3;i++)
				 {
					 ...
					 float wtAVG=1*img(x-1,y-1,i)+1*img(x,y-1,i)+1*img(x+1,y-1,i)+1*img(x-1,y,i)+1*img(x,y,i)+1*img(x+1,y,i)+1*img(x-1,y+1,i)+1*img(x,y+1,i)+1*img(x+1,y+1,i);
					 workimg(x,y,i)=wtAVG/9;
	 ...
	 // CImgDisplay dispimg(img,"new image");
